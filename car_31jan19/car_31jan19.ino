...
void loop() {  
  ...
  delay(10);
#ifdef    AUTONOMOUS  
  prevForwardDist = lookAheadForward(); 
//  readForwardSonic(CENTER_ANGLE);  // read distance looking forward 
  ...
  if (prevForwardDist < COLL_DIST) { // if forward is blocked, move back. let user decide. 
    ...
    displaytext(0/*COLL_DIST*/,F(CARLABEL),str_length20_3);
    #ifdef SERIALL
...
uint8_t lookAheadForward() { // read the ultrasonic sensor distance in cm
  uint8_t leftDistance, rightDistance, centerDistance; //distances on either side
  uint8_t leftorright;
...
void findClearPath() { // if forward path blocked
  ...
  delay(250);
  prevForwardDist = readForwardSonic(CENTER_ANGLE);
  delay(250);
  prevForwardLeftDist = readForwardSonic(LEFT_ANGLE); //set left distance
  delay(250);
  prevForwardDist = readForwardSonic(CENTER_ANGLE);
  delay(250);
  prevForwardDist = (prevForwardLeftDist < prevForwardRightDist) ? (prevForwardLeftDist) : (prevForwardRightDist);
}
//-------------------------------------------------------------------------------------------------------------------------------------
